...
class Reward(object):
    ...
    def __init__(self, reward_scale=1.0,  fuel_coeff=-0.01, hit_coeff = 10.0, 
                 ...
                 tracking_coeff=1.00, tracking_bias=0.00, debug=False,
    ...
    def get(self, missile,  action, done, steps):
        ...
        if not missile.sensor.check_for_vio():
            ...
            r_tracking = self.tracking_coeff * np.exp(-tracking_error**2/self.tracking_sigma**2 - optflow_error**2/self.optflow_sigma**2)
        ...
        reward_info = {}
        r_fuel = self.fuel_coeff * np.sum(np.abs(missile.state['bf_thrust'])) / (missile.actuator_model.fuel_reward_scale)
        ...
        missile.trajectory['att_reward'].append(r_att)
        missile.trajectory['att_penalty'].append(att_penalty)
