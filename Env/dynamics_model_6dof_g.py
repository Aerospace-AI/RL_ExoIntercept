...
class Dynamics_model_6dof(object):
    ...
    def __init__(self, h=0.5, noise_u=np.zeros(3), noise_sd=np.zeros(3), GM=3.986004418e14, R=6.3781e6, theta=0.0, phi=0.0):  
        self.h = h 
        self.g_o = 9.81
        self.GM = GM
        self.R = R
    ...
    def next(self, t , agent):
        ...
        # gravity
        base_position_x = self.R * np.sin(self.theta)*np.cos(self.phi)
        base_position_y = self.R * np.sin(self.theta)*np.sin(self.phi)
        base_posiiton_z = self.R * np.cos(self.theta)

        base = np.asarray([base_position_x, base_position_y, base_posiiton_z]) 
        adj_position = agent.state['position'] + base 
        radial_dist =  np.linalg.norm(adj_position)
        ...
        g = self.GM * adj_position / radial_dist**3
        #print('g: ', np.linalg.norm(g), self.GM , radial_dist/1000, np.linalg.norm(base)/1000) 
        disturbance = g + noise
