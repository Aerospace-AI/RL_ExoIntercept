...
class Reward(object):
    ...
    def __init__(self, reward_scale=1.0,  fuel_coeff=-0.01, hit_coeff = 10.0, 
                 hit_rlimit=2.0, tracking_sigma=0.05, optflow_sigma=0.005, att_limit=np.pi/16, 
    ...
    def get(self, missile,  action, done, steps):
        ...
        if not missile.sensor.check_for_vio():
            ...
            att_error = missile.attitude_parameterization.distance(missile.state['attitude'], missile.state['target_attitude'])
