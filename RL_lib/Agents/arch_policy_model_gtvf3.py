...
class Arch(object):
    def __init__(self, gt_func=None, spred_func=None, vf_traj=None, wait=5):
    ...
    def run_episode(self, env, policy, val_func, model, recurrent_steps):
        ...
        model_spred = model.get_initial_predict(vector_obs)
        flag = 1
        action = np.zeros((1,3))
        env_action = action
        cnt = 0
        while not done:
            ...
            if cnt > self.wait:
                action, env_action, policy_state = policy.sample(None, gt_obs, policy_state)
            cnt += 1
            ...
            model_predict,  model_spred, model_state, model_error = model.predict(action,  model_nobs, model_state, model_error, np.asarray([1]), np.asarray([flag]))

            if self.vf_traj is not None:
                self.vf_traj['value'].append(vpred.copy())
                self.vf_traj['spred'].append(np.squeeze(model_spred.copy()))
