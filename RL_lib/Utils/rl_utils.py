...
class Image_scaler(object):
    ...
    def __init__(self, obs_dim, axes=(0,2,3), channels=2):
        """
        Args:
            obs_dim: dimension of axis=1
        """
        self.vars = np.zeros(channels)
        self.means = np.zeros(channels) 
        self.m = 0
        self.n = 0
        self.first_pass = True
        self.axes = axes
    ...
    def reverse(self,obs):
        for c in range(obs.shape[1]):
            scale = 1/(np.sqrt(self.vars[c]) + 0.1)/3
            means = self.means[c]
            channels.append(obs[:,c,:,:] / scale + means)
        image = np.stack(channels,axis=-3)
        return image 
